...
pGetSYM :: ParseNode -> M.Maybe String
pGetSYM (SYM s) = M.Just s
pGetSYM _       = M.Nothing
...
-- Evaluate a parse tree as a float
evalTree :: ParseTree -> Map.Map String Float -> Float
evalTree tree m
  | pIsNUM tree   = (read . M.fromJust . pGetNUM) tree :: Float
  | pIsSYM tree   = if Map.member sym m then symGetVal else error symError
  | otherwise     = foldl1 fn $ map (\x -> evalTree x m) (pElems tree)
  where sym       = M.fromJust $ pGetSYM tree
        symGetVal = M.fromJust $ Map.lookup sym m
        symError  = "Unknown symbol " ++ sym
        fn        = case (pOperation tree) of
                      "+" -> (+)
                      "-" -> (-)
                      "*" -> (*)
                      "/" -> (/)
...
eval str bindings = evalTree (parse str) $ Map.fromList bindings
